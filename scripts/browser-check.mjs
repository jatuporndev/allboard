...
await page.addInitScript(()=>{
 ...
 window.Worker=class extends NativeWorker{
  constructor(url,options){
   super(url,options);
   if(String(url).includes('bot-worker')){
    this.record={active:true};window.botSearches.push(this.record);
    this.addEventListener('message',e=>{this.record.analysis=e.data.analysis;});
   }
  }
  postMessage(data,...rest){if(this.record)this.record.difficulty=data.difficulty;super.postMessage(data,...rest);}
...
if(!await page.locator('#pause-dialog').isVisible())await page.locator('#pause-game').click();await page.locator('#return-menu').click();await page.locator('#start-menu').click();await page.locator('#solo-game').click();
if(!await page.locator('#playable-game').isVisible()||!await page.locator('#casual-game').isVisible()||!await page.locator('#devil-game').isVisible())throw Error('Difficulty selection missing');
if(/\d+\s*\/\s*10/.test(await page.locator('#difficulty-menu').innerText()))throw Error('Numeric difficulty ratings remain');
if(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth))throw Error('Difficulty menu overflows mobile');
await page.locator('#difficulty-back').click();
if(!await page.locator('#solo-game').isVisible())throw Error('Difficulty back failed');
await page.locator('#solo-game').click();await page.locator('#casual-game').click();await page.waitForTimeout(1400);
if(await page.locator('#solo-difficulty').innerText()!=='Casual')throw Error('Casual label missing');
...
if(await page.evaluate(()=>window.botSearches.at(-1).difficulty)!=='casual')throw Error('Casual search preset missing');
...
await page.locator('#start-menu').click();await page.locator('#solo-game').click();await page.locator('#devil-game').click();await page.waitForTimeout(1400);
if(await page.locator('#solo-difficulty').innerText()!=='Devil')throw Error('Devil label missing');
await clickSquare(20);await clickSquare(28);
await page.waitForFunction(()=>window.botSearches.some(s=>s.active&&s.difficulty==='devil'),{},{timeout:20000});
await page.locator('#pause-game').click();
if(await page.evaluate(()=>window.botSearches.some(s=>s.active)))throw Error('Devil worker did not stop');
await page.locator('#new-game').click();await page.locator('#confirm-new').click();
if(await page.locator('#move-number').innerText()!=='00 MOVES')throw Error('Devil reset failed');
if(await page.locator('#solo-difficulty').innerText()!=='Devil')throw Error('Reset lost Devil difficulty');
await page.locator('#resume-game').click();await clickSquare(20);await clickSquare(28);
await page.waitForFunction(()=>document.querySelector('#move-number').textContent==='02 MOVES'&&!document.querySelector('#new-game').disabled,{},{timeout:30000});
if(await page.evaluate(()=>window.botSearches.at(-1).difficulty)!=='devil')throw Error('Reset lost Devil search preset');
await page.locator('#pause-game').click();await page.locator('#undo').click();
if(await page.locator('#move-number').innerText()!=='00 MOVES')throw Error('Devil undo failed');
await page.locator('#return-menu').click();await page.locator('#start-menu').click();await page.locator('#solo-game').click();await page.locator('#playable-game').click();await page.waitForTimeout(1400);
if(await page.locator('#solo-difficulty').innerText()!=='Playable')throw Error('Playable label missing');
await clickSquare(20);await clickSquare(28);
await page.waitForFunction(()=>document.querySelector('#move-number').textContent==='02 MOVES'&&!document.querySelector('#new-game').disabled,{},{timeout:20000});
if(!await page.evaluate(()=>window.botSearches.at(-1).difficulty==='playable'&&window.botSearches.at(-1).analysis.depth===2))throw Error('Playable did not use the original two-ply bot');
await page.locator('#pause-game').click();await page.locator('#undo').click();
if(await page.locator('#move-number').innerText()!=='00 MOVES')throw Error('Playable undo failed');
await page.locator('#return-menu').click();await page.locator('#start-menu').click();await page.locator('#local-game').click();
if(await page.locator('#solo-difficulty').isVisible())throw Error('Solo difficulty leaked into local mode');
await page.locator('#pause-game').click();await page.locator('#return-menu').click();
