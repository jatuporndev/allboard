...
The game opens in a cinematic temple menu. Choose **Start Game → Solo**, then **Playable**, **Casual**, or **Devil** to play Ivory against the Ember King bot, or choose **Local 2 Player** to share a device. The selected difficulty appears beneath the bot's player plaque and stays selected through pause, undo, and New Game. Return to the main menu to choose another difficulty. All three choices support English and Thai. Difficulty is shown by name only.

### How the Ember King thinks

Playable restores the original two-ply bot: it compares its next move and the opponent's immediate reply using material and simple positional bonuses. It runs in the same cancellable background worker, without deeper capture analysis.

Casual and Devil search increasingly deep lines using negamax with alpha-beta pruning, retaining the best move from the last completed iteration. Casual preserves the existing bot: a 1.8-second search budget, a maximum normal depth of six plies (a ply is one player's move), and a 150,000-node safety limit. Devil allows six seconds, ten plies, and 600,000 nodes. These are active calculation budgets, not fixed waits. Actual depth depends on the position and device. Captures, promotions, and the previous iteration's preferred moves are examined first. Devil also prioritizes quiet moves that refuted earlier lines using killer and history move-ordering heuristics. This changes search order without omitting legal moves or altering the game rules.

At the search horizon, quiescence search follows captures and promotions for up to six additional plies in Casual or eight in Devil, and extends check evasions instead of treating check as a position where the bot can pass. Long check sequences have a safety cap. Both modes share an evaluation of material, central control, mobility, development, pawn advancement and promotion prospects, and king centralization in low-material endings. Promoted Bia have Met strength. Checkmate outranks material, with faster mates preferred.
