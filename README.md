...
The game opens in a cinematic temple menu. Choose **Start Game → Solo**, then **Casual · 5/10** or **Devil · 10/10** to play Ivory against the Ember King bot, or choose **Local 2 Player** to share a device. The selected difficulty appears beneath the bot's player plaque and stays selected through pause, undo, and New Game. Return to the main menu to choose another difficulty. Both choices support English and Thai. These numbers label relative difficulty within the game; they are not measured strength or Elo ratings.

### How the Ember King thinks

The bot searches increasingly deep lines using negamax with alpha-beta pruning, retaining the best move from the last completed iteration. Casual preserves the existing bot: a 1.8-second search budget, a maximum normal depth of six plies (a ply is one player's move), and a 150,000-node safety limit. Devil allows six seconds, ten plies, and 600,000 nodes. These are active calculation budgets, not fixed waits. Actual depth depends on the position and device. Captures, promotions, and the previous iteration's preferred moves are examined first. Devil also prioritizes quiet moves that refuted earlier lines using killer and history move-ordering heuristics. This changes search order without omitting legal moves or altering the game rules.
