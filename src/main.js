...
function refresh(){world.highlight(selected,moves,store.state);renderUI(store,selected,moves);if(gameMode==='solo'&&store.state.turn==='black'&&!store.state.result){$('status').textContent='The Ember King is thinking...';$('black-turn').textContent='BOT TURN';}const locked=animator.busy;$('undo').disabled=locked||!store.snapshots.length;$('new-game').disabled=locked;$('return-menu').disabled=locked;$('count').disabled=locked||(gameMode==='solo'&&store.state.turn==='black');$('accept-draw').disabled=$('count').disabled;}
...
function playMove(from,to){const next=store.dispatch({type:'MOVE',from,to});animator.move(next.history.at(-1),()=>{world.sync(next);refresh();if(next.result)toast(next.result.winner?`${next.result.winner==='white'?'Ivory':'Obsidian'} wins: ${next.result.reason}`:`Draw: ${next.result.reason}`);else if(inCheck(next.board,next.turn))toast('Check - protect your Khun.');else if(next.history.at(-1).promoted)toast('Bia awakened - promoted to Bia Ngai.');scheduleBot();});refresh();}
...
let last=performance.now(),elapsed=0;function frame(now){const dt=Math.min((now-last)/1000,.04);last=now;elapsed+=dt;world.update(elapsed);if(!isPaused()){animator.update(dt);effects.update(dt);}if(playing){if(!isPaused())rig.update(dt,elapsed);}else{const t=menuMotion?elapsed:0;world.camera.position.set(7+Math.sin(t*.09)*1.3,4.4+Math.sin(t*.12)*.25,12.5);world.camera.lookAt(0,3,-7);}world.render();requestAnimationFrame(frame);}requestAnimationFrame(frame);
...
function startGame(mode){clearTimeout(botTimer);gameMode=mode;playing=true;document.body.classList.remove('in-menu');$('main-menu').hidden=true;store.reset();cameraMode('orbit');rig.home();document.querySelector('.live-label').textContent=mode==='solo'?'SOLO / VS THE EMBER KING':'LOCAL TWO-PLAYER';document.querySelector('.black h3').textContent=mode==='solo'?'The Ember King':'Obsidian dynasty';world.resize();rig.home();$('pause-game').focus();}
$('solo-game').onclick=()=>startGame('solo');$('local-game').onclick=()=>startGame('local');
$('return-menu').onclick=()=>{if(animator.busy)return;clearTimeout(botTimer);playing=false;$('pause-dialog').close();document.exitPointerLock?.();rig.keys.clear();rig.orbit.enabled=false;selected=null;moves=[];world.highlight(null,[],store.state);document.body.classList.add('in-menu');$('main-menu').hidden=false;showModes(false);world.resize();$('start-menu').focus();};
...
window.addEventListener('resize',()=>{if(playing&&rig.mode==='orbit'){world.resize();rig.home();}});
