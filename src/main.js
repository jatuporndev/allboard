...
function playMove(from,to){if(gameMode==='online'){sendOnline({type:'MOVE',from,to});return;}const next=store.dispatch({type:'MOVE',from,to});animator.move(next.history.at(-1),()=>{world.sync(next);refresh();if(next.result)toast(next.result.winner?`${next.result.winner==='white'?'Ivory':'Obsidian'} wins: ${next.result.reason}`:`Draw: ${next.result.reason}`);else if(inCheck(next.board,next.turn))toast('Check - protect your Khun.');else if(next.history.at(-1).promoted)toast('Bia awakened - promoted to Bia Ngai.');scheduleBot();});refresh();}
...
function receiveRoom(room){
  if(gameMode!=='online'||!playing||activeRoom!==online.code){startGame('online');activeRoom=online.code;appliedRevision=-1;appliedResignation=null;}
  ...
  const finish=()=>{world.sync(next);refresh();if(next.result)toast(next.result.winner?next.result.winner+' wins: '+next.result.reason:'Draw: '+next.result.reason);if(queuedRoom){const room=queuedRoom;queuedRoom=null;receiveRoom(room);}};
  if(animate){animator.move(next.history.at(-1),finish);refresh();}else finish();
}
mountOnline({onPreview:show=>{multiplayerPreview=show;playing=false;rig.keys.clear();rig.orbit.enabled=show;document.body.classList.toggle('in-menu',!show);$('main-menu').hidden=show;if(show){clearTimeout(botTimer);document.exitPointerLock?.();cameraMode('orbit');store.reset();world.resize();rig.home({immediate:true});}else{$('multiplayer-menu').focus();}},onMatch:receiveRoom,onConnection:()=>refresh(),onExit:()=>{if(gameMode==='online'){playing=false;gameMode='local';document.body.classList.add('in-menu');$('main-menu').hidden=false;}}});
mountLanguage();
